...
/*
  ...
  GPIOA_AHB->DR2R |=  ((1ul << 6) | (1ul << 7)); /// PJ0, PJ1 2-mA Drive       /
  GPIOA_AHB->PUR  |=  ((1ul << 6) | (1ul << 7)); /// PJ0, PJ1 pull-up          /
  GPIOA_AHB->DIR  &= ~((1ul << 6) | (1ul << 7)); /// PJ0, PJ1 is intput        /
  GPIOA_AHB->DEN  |=  ((1ul << 6) | (1ul << 7)); /// PJ0, PJ1 is digital func. /
}
...
uint32_t BTN_Get (void) {
  ...
  //return (~(GPIOJ_AHB->DATA) & ((1ul << 0) | (1ul << 1)));
		return ((GPIOA_AHB->DATA) & ((1ul << 6) | (1ul << 7)));//depend on pull up~-down meiyou~
}